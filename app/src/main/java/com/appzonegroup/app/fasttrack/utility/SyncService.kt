...
//        updateChecker.run()
    ...
    private fun startMiddlewareNotifications() {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            val url =
                "${ApiServiceObject.BASE_URL}/${ApiServiceObject.STATIC}/POSCashOutNotification"
            val serializer = Gson()
    ...
    internal inner class SyncClass : Runnable {
        ...
        //Context context;
        var gson: Gson = Gson()
        var agentPhone = LocalStorage.GetValueFor(AppConstants.AGENT_PHONE, baseContext)
        var agentPIN = LocalStorage.GetValueFor(AppConstants.AGENT_PIN, baseContext)
        var institutionCode = LocalStorage.GetValueFor(AppConstants.INSTITUTION_CODE, baseContext)
