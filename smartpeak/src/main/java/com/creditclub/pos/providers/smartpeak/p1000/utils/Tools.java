package com.creditclub.pos.providers.smartpeak.p1000.utils;
                ...
                switch (tlv.getTag()) {
                    case PosCardManager.MAGCARD_TRACK_TLV_TAG_1:
                        ...
                        if (tlvData != null && tlvData.length > 0) {
                            track[0] = new String(tlvData, StandardCharsets.ISO_8859_1);
                        }
                        break;
                    case PosCardManager.MAGCARD_TRACK_TLV_TAG_2:
                        tlvData = tlv.getData();
                        if (tlvData != null && tlvData.length > 0) {
                            track[1] = new String(tlvData, StandardCharsets.ISO_8859_1);
                        }
                        break;
                    case PosCardManager.MAGCARD_TRACK_TLV_TAG_3:
                        tlvData = tlv.getData();
                        if (tlvData != null && tlvData.length > 0) {
                            track[2] = new String(tlvData, StandardCharsets.ISO_8859_1);
                        }
                        break;
                    default:
                        break;
