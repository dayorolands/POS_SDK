...
object Dukpt {
    ...
    private val DEFAULT_KEY_REGISTER_BITMASK =
        toBitSet(toByteArray(KEY_REGISTER_BITMASK))
    ...
    private val DEFAULT_VARIANT_BITMASK =
        toBitSet(toByteArray(PIN_VARIANT_BITMASK))
    ...
    fun computeKey(
        baseDerivationKey: ByteArray,
        keySerialNumber: ByteArray
    ): ByteArray {
        return computeKey(
            baseDerivationKey,
            keySerialNumber,
            DEFAULT_KEY_REGISTER_BITMASK,
            DEFAULT_VARIANT_BITMASK
        )
    }
    ...
    internal fun computeKey(
        baseDerivationKey: ByteArray,
        keySerialNumber: ByteArray,
        keyRegisterBitmask: BitSet?,
        dataVariantBitmask: BitSet
    ): ByteArray {
        val bdk = toBitSet(baseDerivationKey)
        val ksn = toBitSet(keySerialNumber)
        val ipek = getIpek(bdk, ksn, keyRegisterBitmask)

        // convert key for returning
        val key =
            _getCurrentKey(ipek, ksn, keyRegisterBitmask, dataVariantBitmask)
     ...
     */
    @Throws(Exception::class)
    internal fun getIpek(
        key: BitSet,
        ksn: BitSet,
        keyRegisterBitmask: BitSet?
    ): BitSet {
        val ipek = arrayOfNulls<ByteArray>(2)
        val keyRegister = key[0, key.bitSize()]
        val data = ksn[0, ksn.bitSize()]
        data.clear(59, 80)
        ipek[0] =
            encryptTripleDes(toByteArray(keyRegister), toByteArray(data[0, 64]))
        keyRegister.xor(keyRegisterBitmask)
        ipek[1] =
            encryptTripleDes(toByteArray(keyRegister), toByteArray(data[0, 64]))
        val bipek = concat(ipek[0], ipek[1])
        val bsipek = toBitSet(bipek)
     ...
     */
    @Throws(Exception::class)
    private fun _getCurrentKey(
        ipek: BitSet,
        ksn: BitSet,
        keyRegisterBitmask: BitSet?,
        dataVariantBitmask: BitSet
    ): BitSet {
        var key = ipek[0, ipek.bitSize()]
        val counter = ksn[0, ksn.bitSize()]
        counter.clear(59, ksn.bitSize())
        for (i in 59 until ksn.bitSize()) {
            if (ksn[i]) {
                counter.set(i)
                val tmp = _nonReversibleKeyGenerationProcess(
                    key,
                    counter[16, 80],
                    keyRegisterBitmask
                )
     ...
     */
    @Throws(Exception::class)
    private fun _nonReversibleKeyGenerationProcess(
        p_key: BitSet,
        data: BitSet,
        keyRegisterBitmask: BitSet?
    ): BitSet {
        ...
        var reg2 = reg1[0, 64] // reg2 is being used like a temp here
        reg2.xor(keyreg[64, 128]) // and here, too, kind of
        // step 2: Crypto Register-2 DEA-encrypted using, as the key, the left half of the Key Register goes to Crypto Register-2
        reg2 = toBitSet(
            encryptDes(
                toByteArray(keyreg[0, 64]),
                toByteArray(reg2)
            )
        )
    ...
    fun encryptDes(
        ...
        val encryptKey = SecretKeyFactory.getInstance("DES")
            .generateSecret(DESKeySpec(key))
    ...
    fun toDataKey(derivedKey: ByteArray?): ByteArray {
        ...
        val dataKey = concat(leftEncrypted, rightEncrypted)
        obliviate(left)
        obliviate(right)
    ...
    fun decryptDes(
        ...
        val iv =
            IvParameterSpec(ByteArray(8))
        val decryptKey = SecretKeyFactory.getInstance("DES")
            .generateSecret(DESKeySpec(key))
    ...
    @JvmOverloads
    @Throws(Exception::class)
    fun encryptTripleDes(
        key: ByteArray,
        data: ByteArray?,
        padding: Boolean = false
    ): ByteArray {
        val bskey = toBitSet(key)
        val k1: BitSet
        val k2: BitSet
        val k3: BitSet
        if (bskey.bitSize() === 64) {
        ...
        val kb1 = toByteArray(k1)
        val kb2 = toByteArray(k2)
        val kb3 = toByteArray(k3)
        val key16 = concat(kb1, kb2)
        val key24 = concat(key16, kb3)
        val iv =
            IvParameterSpec(ByteArray(8))
        val encryptKey = SecretKeyFactory.getInstance("DESede")
    ...
    fun decryptTripleDes(
        ...
        val decryptor: Cipher
        decryptor =
            if (padding) Cipher.getInstance("DESede/CBC/PKCS5Padding") else Cipher.getInstance("DESede/CBC/NoPadding")
    ...
    fun toBitSet(b: Byte): BitSet {
        val bs = BitSet(8)
        for (i in 0..7) {
            if (b.toInt() and (1L shl i).toInt() > 0) {
                bs.set(7 - i)
            }
        }
        return bs
    ...
    fun toBitSet(b: ByteArray): BitSet {
        ...
        for (i in b.indices) {
            for (j in 0..7) {
                if (b[i].toInt() and (1L shl j).toInt() > 0) {
                    bs.set(8 * i + (7 - j))
                }
            }
        }
    ...
    fun toByte(b: BitSet): Byte {
        ...
        for (i in 0 until b.bitSize()) {
            if (b[i]) value = (value.toInt() or (1L shl 7 - i).toInt()) as Byte
        }
        return value
    }
    ...
    fun toByteArray(b: BitSet): ByteArray {
        val size = Math.ceil(b.bitSize() / 8.0).toInt()
        val value = ByteArray(size)
        for (i in 0 until size) {
            value[i] = toByte(b[i * 8, Math.min(b.bitSize(), (i + 1) * 8)])
        }
        return value
    }
     ...
     */
    @JvmOverloads
    fun obliviate(b: BitSet, n: Int = NUM_OVERWRITES) {
        val r = SecureRandom()
        for (i in 0 until NUM_OVERWRITES) {
     ...
     */
    @JvmOverloads
    fun obliviate(b: ByteArray?, n: Int = NUM_OVERWRITES) {
        for (i in 0 until n) {
            b!![i] = 0x00
            b[i] = 0x01
        }
    ...
    private fun BitSet.bitSize() = size()
}
